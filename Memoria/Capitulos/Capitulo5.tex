...
Todas las imágenes y vídeos LSE se han descargado del catálogo de recursos de ARASAAC\footnote{\url{http://www.http://www.arasaac.org/descargas.php}}, que cuenta con 4.138 imágenes en formato jpg, y 4.102 vídeos en formato mp4. Estos recursos los hemos almacenado en dos carpetas alojadas en el propio servidor holstein: carpeta ``imagenes'' y carpeta ``videos''. Los servicios de traducción de palabra a vídeo LSE y a imagen LSE permiten obtener estos recursos mediante una llamada GET, en la cual se accede a la carpeta correspondiente y se busca el recurso con el nombre del signo buscado, concatenado con el formato que corresponda (jpg en el caso del servicio de traducción a imagen y mp4 en el de traducción a vídeo). Para crear la respuesta tanto en imágenes como en vídeo, se ha utilizado la función \textit{``sendfile''} de Flask, que indicando como parámetro el fichero y su formato permite devolver archivos transformados en formato JSON.
...
En el parámetro \textit{``texto''} se debe incluir el texto que se desea traducir a LSE. Al recibir este parámetro, este servicio lo utilizará para hacer una llamada al servicio de traducción de texto en LN a texto LSE adaptado al catálogo de imágenes de ARASAAC, con el fin obtener la traducción a texto LSE. Una vez obtenida esta traducción, por cada palabra se llamará al servicio de traducción de palabra a imagen LSE, que devolverá la ruta de la imagen correspondiente o la imagen de error en caso de que no exista la imagen buscada. Al finalizar con todas las palabras se añadirán sus rutas en orden en un array JSON que se devolverá como respuesta. El flujo de este servicio lo podemos observar en la Figura~\ref {fig: FlujoImagenTexto}.\\

Por ejemplo, para obtener las imágenes en LSE del texto \textit{``Los niños comen chocolate''}, se debe realizar la llamada POST indicada anteriormente con el siguiente JSON:
...
La respuesta recibida será un JSON que contendrá un array con todas las rutas de las imágenes, como podemos ver a continuación. COMPLETAR EJEMPLO
