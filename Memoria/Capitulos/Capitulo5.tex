...
Respecto a la parte del servidor, se han desarrollado siete servicios web distintos: dos para devolver de manera rápida el vídeo y la imagen en LSE de una sola palabra, otro para devolver el texto traducido a texto en LSE, dos para devolver el texto LSE adaptado a los vídeos e imágenes de ARASAAC, y otros dos para devolver el texto traducido a video e imágenes en LSE. \\
...
En este proyecto, se ha configurado el servidor Nginx como servidor proxy inverso con el fin de redirigir las distintas solicitudes al servidor correspondiente. Se han especificado una serie de rutas para diferenciar qué llamadas de usuario redireccionar a la página web y cuáles a los servicios web. Esta estructura se puede observar en la Figura~\ref {fig: imgProxy}.
...
Para que el servicio web sea capaz de traducir una frase a texto LSE, se debe que realizar un procesamiento previo del texto en castellano para poder adaptarlo a la Lengua de Signos Española. Este procesamiento es realizado con un sistema de reglas, que filtran qué palabras deben de ser utilizadas y cuáles no, al igual que determina su orden en la oración en LSE, puesto que la estructura gramatical de la LSE es diferente al castellano, tal y como se contó en la sección ~\ref {cap2:sec:Sintaxis de la Lengua de Signos}. La elección a la hora de desarrollar el Procesamiento del Lenguaje Natural para nuestro proyecto estaba entre un sistema basado en reglas o mediante el uso de aprendizaje automático. La opción de desarrollar un aprendizaje automático no era viable debido a la dificultad y la falta de ejemplos en castellano para entrenar el sistema. Por este motivo se eligió el sistema basado en reglas, ya que era más fácil de implementar y, aunque puede limitar la capacidad de traducción de frases que no cumplan dichas reglas, cumple con el objetivo marcado en el proyecto, que es traducir frases simples a LSE.\\
...
Para poder realizar el análisis sintáctico y morfológico de la oración se necesita obtener toda la información posible de la oración. Para ello, se realizará con Spacy el proceso de tokenización, el cual divide la oración en una lista de palabras denominadas \textit{tokens}, donde cada token contiene la siguiente información de cada palabra:

\begin{itemize}
		
	 \item \textbf{Dependencia:} Indica la relación de dependencia sintáctica dentro de la oración, es decir, qué función tiene la palabra dentro de la oración. Las funciones principales son sujeto, raíz (que es la palabra de la que dependen todas las demás) u objeto. En la Figura ~\ref {fig: raizEjemplo} se puede ver un ejemplo donde ``\textit{yo}'' es el sujeto, el verbo ``\textit{comer}'' la raíz y ``\textit{chocolate}'' el objeto. 
...
Por ejemplo, los tokens de la frase \textit{``Los niños merendaron chocolate''} serían los que aparecen en la Figura ~\ref {fig: tokenInformacion}. Como se puede ver, cada token tiene su información detallada. Por ejemplo, de los tokens \textit{``los''} y \textit{``niños''} la información obtenida sería:
\begin{itemize}
...
\begin{figure}[h]
	...
	\caption{ Tokens de la frase \textit{``Los niños meredaron chocolate''} }
	\label {fig: tokenInformacion}
\end{figure}


Durante el proceso de tokenización también se obtiene un árbol de dependencias entre palabras. Para entender cómo funcionan las dependencias se va a utilizar como ejemplo la oración \textit{``Ayer los niños merendaban chocolate''}, donde sus dependencias se muestran en la Figura ~\ref {fig: dependencias} 
...
Como se puede apreciar en el diagrama del token, de la palabra \textit{``merendaban''}, que es la raíz de la oración, dependen tres tokens (Ayer, niños y chocolate), y de ellos también dependen otros y así sucesivamente. Estas dependencias se traducen en un árbol, donde el ROOT es la raíz, como podemos ver en la Figura~\ref {fig: grafo}:\\
...
\begin{itemize}
	...
	\item \textbf{Determinantes:} Es importante detectar estos elementos, ya que los determinantes se omiten a la hora de hacer la traducción. Por ejemplo,  \textit{``El niño come carne''} se traduce a LSE como \textit{``NIÑO CARNE COMER''.}
	
	\item \textbf{Verbos copulativos:} Los verbos copulativos ser, estar o parecer se omiten a la hora de la traducción a texto LSE. Por ejemplo, \textit{``yo soy bajo''} se traduce como \textit{``YO BAJO''.}
	
	\item \textbf{Posesivos:} Los determinantes posesivos se sustituyen por pronombres personales. Por ejemplo, \textit{``Mi niño es bajo''} se traduce como \textit{``YO NIÑO BAJO''.}
	
	
	\item \textbf{Adjetivos:} Al traducir una frase a LSE, los adjetivos siempre van en masculino. Por ejemplo, \textit{``Mi mamá es fea''} se traduce como \textit{``YO MAMÁ FEO''.}
	
	\item \textbf{Temporalidad:} En la LSE, la temporalidad va al principio de la oración. Por ejemplo, \textit{``Yo comí chocolate ayer''} se traduce como \textit{``AYER YO CHOCOLATE COMER''.} Sin embargo, no siempre hay adverbios de tiempo en la oración que indiquen la temporalidad. En ese caso, el tiempo viene determinado por el tiempo verbal. Por ejemplo, \textit{``Yo comeré chocolate''} se traduce como \textit{``FUTURO YO CHOCOLATE COMER''.}
...
El texto en LSE final que devolvería el servicio en este caso sería: ``AYER NIÑOS CHOCOLATE MERENDAR''
