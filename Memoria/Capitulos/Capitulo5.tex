...
En esta sección se explican con detalle los servicios web desarrollados, así como los métodos utilizados en cada uno de ellos. Para comenzar COMPLETAR
...
\subsubsection{Implementación del PLN}
...
\subsubsection{Desarrollo del PNL}
El desarrollo del Procesamiento del Lenguaje Natural está dividido en tres partes diferenciadas que son:\\


\textbf{Recopilar la  información:} para poder empezar con el desarrollo se necesita conocer la información de cada palabra dentro de la oración. Para ello se realizará el proceso de Tokenización el cual divide la frase en una lista de palabras las cuales pasan a denominarse ?tokens? pero con la diferencia de que ahora cada token contiene gran cantidad de información. Esta información es:
...
\textbf{Análisis sintáctico y orden de la oración:} una vez separado la oración en tokens hay que diferenciar cuales forman parte del sujeto y cuales del predicado. Para ello hay que recorrer el árbol de dependencias  generado por Spacy mediante una función recursiva empezando desde el ROOT que es el token raíz del cual dependen todos los demás. Para entender cómo funcionan las dependencias se va a utilizar como ejemplo la oración `Ayer los niños merendaban chocolate'.
...
\textbf{Análisis morfológico:} Una vez realizado el análisis sintáctico y orden de la oración hay que hacer un análisis morfológico detectando el tiempo verbal de la oración. el género y el número de cada token, así como los determinantes posesivos los cuales son sustituidos por pronombres personales.\\
