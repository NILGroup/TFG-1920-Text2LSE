...
\begin{itemize}
	...
	\item \textbf{Particularidad LSE:} En LSE existen frases hechas que se dicen con un único signo concreto, por lo que es incorrecto realizar la traducción de la frase hecha palabra por palabra. Por ejemplo, la oración \textit{``saluda dando la mano''} es una oración hecha y en LSE se traduce como un solo signo que representa saludar dando la mano.
	...
	\item \textbf{Fallo del analizador Spacy:} Text2LSE ha utilizado el analizador Spacy, el cual no tiene un porcentaje de acierto del 100\%. Por tanto, puede haber ocasiones en que el análisis Spacy no sea correcto y esto lleve a nuestro sistema a fallar en la traducción. Esto lo podemos comprobar en oraciones como \textit{``Solo es barro''}, ya que la palabra \textit{``barro''} no la toma como sustantivo, sino como verbo \textit{(``barrer'')}, generando la traducción \textit{``SOLO BARRER''}, que es incorrecta.
...
Por otro lado, se han analizado todas las traducciones que cuyo texto LSE es entendible, haciendo un recuento de los gestos que se buscan y viendo qué porcentaje de signos no se encuentran en el catálogo de ARASAAC, con el fin de dar una idea de qué parte de fallos son debidos a la falta de signos en el catálogo. En la tabla \ref{tabla:resultadoGestos} se muestra el número de gestos buscados en el catálogo de ARASAAC, el número de gestos que no se encuentran y el porcentaje que este representa.\\

\begin{table}[H]
	\begin{center}
		\begin{tabular}{|m{4cm}|M{2.1cm}|M{2.3cm}|M{2.1cm}|}
			\hline
			\textbf{Corpus}  & \textbf{Nº gestos buscados} & \textbf{Nº gestos no encontrados}  & \textbf{\% error}  \\
			\hline \hline
